...
`--rewrite` asks a chat model on the same endpoint to restate the question in codebase vocabulary,
embeds that restatement too, and scores each place with the better of its two cosines; the answer
carries the restatement as `rewrite`. It needs a chat model served beside the embedding model
(`GRAPHIFY_SEARCH_CHAT_MODEL`, default `qwen3.8-27b`), and it costs one chat call and one extra
embedding per query -- about twenty seconds and six hundred tokens against a local 27B model. If
the chat endpoint does not answer, the query falls back to the question alone and prints no
`rewrite`; under `--require-dense` it is refused instead.
